...
input_all_coins_filename_except_eth = 'allcoinlist.txt'
input_eth_filename = '2020-10-12_eth_balance_100000.txt'
input_bch_filename = 'bitcoin_cash_address.txt'
output_combined_hash160_filename = 'my_combined_altcoin_h160_file.txt'
...
def read_h160_file():
    with open(output_combined_hash160_filename, 'r') as f:
        h160_list = f.read().rstrip('\n').split('\n')
    print('-------------------------------------------------------------------\n')
    return h160_list
            
def save_h160_file(h160_list):
    with open(output_combined_hash160_filename, 'a') as f:
        for line in h160_list:
            f.write(line + '\n')

def read_eth_file():
    with open(input_eth_filename) as f:
        eth_list = f.read().rstrip('\n').split('\n')
    print('\n\nLoaded ' + str(len(eth_list)) +' Ethereum address from the file')
    return eth_list
...
def read_allcoin_address_file():
    ...
    ################# Reading Phase #########################
    with open(input_all_coins_filename_except_eth) as f:
            coin_list = f.read().rstrip('\n').split('\n')
    print('\nLoaded ' + str(len(coin_list)) +' address from file')
    
    bch_list = read_bch_file()
    ################ Preparation Phase #############################
    ...
    legacy_bch_list = [x for x in bch_list if x[0] == '1']  # BCH 1 Address
    segwit_bch_list = [x for x in bch_list if x[0] == '3']  # BCH 3 Address
    bech32_bch_list = [x for x in bch_list if x[0] == 'b' and len(x) < 45]      # BCH bech32. ignore multisig address.
    ...
    doge_list = [x for x in coin_list if x[0] == 'D']  # DOGE coin Address
    xrp_list = [x for x in coin_list if x[0] == 'r']  # XRP coin Address
    ...
    print(str(len(legacy_btc_list))+' Legacy BTC, '
          ...
          +str(len(doge_list))+' DOGE coin, '
          +str(len(doge_list))+' XRP Ripple, '
    ...
    print('\nIgnored : '+str(len(coin_list)-len(legacy_btc_list)-len(segwit_btc_list)-len(bech32_btc_list)
        -len(Legacy_ltc_list)-len(zcash_t1_list)-len(zcash_t3_list)-len(dash_list)-len(doge_list)-len(xrp_list))
    ...
    h160_list.extend([bit.base58.b58decode_check(address)[1:].hex() for address in legacy_bch_list])
    h160_list.extend([bit.base58.b58decode_check(address)[1:].hex() for address in segwit_bch_list])
    h160_list.extend([bech32_to_hash160(address) for address in bech32_bch_list])
    ...
    h160_list.extend([bit.base58.b58decode_check(address)[1:].hex() for address in doge_list])
    h160_list.extend([ripple_address_to_hash160(address) for address in xrp_list])
    save_h160_file(h160_list)
    return h160_list

################ H160 File Check ############################
eth_list = read_eth_file()
eth_list = set(eth_list)

valid = os.path.isfile(output_combined_hash160_filename)
if valid == True:
    print('\nFound the hash160 file: '+output_combined_hash160_filename+'. Will be used directly')
    h160_list = read_h160_file()
else:
    print('\nNot Found '+output_combined_hash160_filename+'. Will convert Address file into hash160. \
...
k = 1
incr_print = 50000
m = random.SystemRandom().randint(1, 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364140)  # 256 bit range
...
while True:
    ...
    eth_addr = ETH_Address(upub)
    
    if crmd.hex() in h160_list or urmd.hex() in h160_list or segwit_rmd.hex() in h160_list or eth_addr in eth_list:
        ...
        if (k % incr_print) == 0:
            m = random.SystemRandom().randint(1, 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364140)  # 256 bit range
            print ('  {:0.2f} Address/s    :: Total Address Searched: {}'.format(4*incr_print/(time.time() - st), 4*k), end='\r')
            st = time.time()
    k += 1
    m += 1
